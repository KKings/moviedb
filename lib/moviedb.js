...
MovieDB.prototype.session = function (fn) {
	...
	request
		.get(endpoints.base_url + endpoints.authentication.sessionId)
...
/**
 * Automatically validates and creates the session
 */
MovieDB.prototype.validateLogin = function (credentials, fn) {
	var self = this;

	request
		.get(endpoints.base_url + endpoints.authentication.validateLogin)
		.query({
...
var execMethod = function (type, params, endpoint, fn) {
	...
	if (data.query && data.query === Object(data.query)) {
		params = data.query;
	}
